...
export async function generateStaticParams() {
  const params: Array<{ locale: Locale; topic: string; page: string }> = [];
  const PAGE_SIZE = DEFAULT_PAGE_SIZE;
  const MAX_STATIC_PAGES_PER_TOPIC = 2;
  ...
  for (const locale of ['en', 'ko'] as const) {
    ...
    for (const topic of topics) {
      ...
      for (let p = 2; p <= Math.min(totalPages, MAX_STATIC_PAGES_PER_TOPIC); p++) {
        params.push({ locale, topic: topic.id, page: String(p) });
      }
    }
  }
...
export async function generateMetadata({
  params: { locale, topic, page },
}: {
  params: { locale: string; topic: string; page: string };
}) {
  ...
  const title = config
    ? locale === 'ko'
      ? `${config.title} 토픽 · ${pageNumber}페이지`
      : `Topic: ${config.title} · Page ${pageNumber}`
    : locale === 'ko'
      ? `토픽 · ${pageNumber}페이지`
  ...
  const url = `${BASE_URL}/${locale}/topics/${encodeURIComponent(normalized)}/page/${pageNumber}`;
  const languageAlternates = {
    en: `${BASE_URL}/en/topics/${encodeURIComponent(normalized)}/page/${pageNumber}`,
    ko: `${BASE_URL}/ko/topics/${encodeURIComponent(normalized)}/page/${pageNumber}`,
  };
...
export default function TopicPageNumber({
  params: { locale, topic, page },
}: {
  params: { locale: string; topic: string; page: string };
}) {
  ...
  if (raw !== normalized) {
    redirect(`/${locale}/topics/${encodeURIComponent(normalized)}/page/${pageNumber}`);
  }
  ...
  const pagePosts = sliceForPage(filtered, pageNumber, DEFAULT_PAGE_SIZE);
  const breadcrumbJsonLd = buildBreadcrumbJsonLd([
    ...
    { name: locale === 'ko' ? `${pageNumber}페이지` : `Page ${pageNumber}`, path: `/${locale}/topics/${encodeURIComponent(topicConfig.id)}/page/${pageNumber}` },
  ]);
