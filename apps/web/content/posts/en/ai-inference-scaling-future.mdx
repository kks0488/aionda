...
Many people talk about the limits of AI, seeing that no groundbreaking model has emerged since GPT-4. But this is seeing only the tip of the iceberg. As OpenAI's o1 model recently demonstrated, it has been proven that performance can be dramatically improved by increasing the **'Test-time Compute'**—the time the model spends thinking before answering.
...
- **System 1 (Intuition)**: Current chatbots that react instantly to questions (e.g., GPT-4o).
...
### Q3. When will GPT-5 be released?
**A.** The exact timing is unknown, but the next-generation model is likely to be one where this "reasoning capability" is internalized, rather than just being a model with "lots of training data." o1 is just a preview showing that direction.
