...
alternateLocale: /en/posts/openai-vs-anthropic-funding-race
...
2. **모델 훈련 비용**: GPT-5와 후속 모델 학습에 $150B 투입. OpenAI는 GPT-5가 현재 대비 100배 성능 향상을 목표로 한다고 밝혔습니다.
...
실제 벤치마크에서 Claude 3.7 Sonnet은 여러 영역에서 GPT-4를 능가합니다:
- MMLU(대학 수준 지식): Claude 88.7% vs GPT-4 86.4%
- HumanEval(코딩): Claude 92% vs GPT-4 87%
- MATH(수학 문제): Claude 78.5% vs GPT-4 76.2%
...
밸류에이션은 기술력이 아니라 **미래 시장 지배력 예측**입니다. OpenAI의 높은 평가는 ChatGPT의 브랜드 파워와 Microsoft 파트너십 때문이지, 모델 성능 때문이 아닙니다. 실제로 여러 벤치마크에서 Claude가 GPT-4를 앞섭니다.

또 다른 오해는 "더 많은 투자 = 더 나은 모델"입니다. OpenAI는 GPT-4 학습에 $100M 이상 썼다고 알려졌지만, Anthropic의 Claude 3.7은 훨씬 적은 비용으로 비슷한 성능을 달성했습니다. Constitutional AI의 효율성 덕분입니다.
...
단기 수익을 원한다면 OpenAI, 장기 안정성을 원한다면 Anthropic입니다. OpenAI는 2024년 매출 $3.7B로 Anthropic($1.2B)의 3배이며, ChatGPT의 강력한 브랜드를 보유합니다. 하지만 $500B 밸류에이션은 연 27배 성장을 전제로 하며, 실패 시 조정 폭이 클 수 있습니다. Anthropic은 $350B 평가액이지만 2024년 850% 성장률과 기술적 우위(Claude 3.7이 여러 벤치마크에서 GPT-4 능가)를 보유합니다. AWS와 Google의 이중 파트너십도 위험 분산에 유리합니다. 전문 투자자들은 포트폴리오에 두 회사를 함께 포함시키는 전략을 선호합니다.
